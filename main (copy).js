	...
	var CENTIPEDE = {
		...
		GAME: {
			...
			over: function (player) {
				...
				var animation = CENTIPEDE.GAME.tools.animation;
			...
			tools: {
				...
				update: function () {
					...
					for (; t < players.length; t++) {
						...
						if (new_x < 0 || new_x + centipede.width > properties.buffer.width) {
							// angle = Math.PI - angle;
							GAME.over(player);
						}
						if (new_y < 0 || new_y + centipede.height > properties.buffer.height) {
						...
						if (collide["centipede-with-itself"](centipede, brick_properties)) {
							GAME.over(player);
						}
		...
		DOM: {
			...
			tools: {
				...
				addPlayer: function () {
					...
					CENTIPEDE.GAME.players.push({"left-dom-key": inputs[0], "right-dom-key": inputs[1], name: player_name, index: new_player_index, centipede: []});
